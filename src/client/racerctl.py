            ...
            if response.get('success', False):
                ...
                if count == 0:
                    ...
                    for container in containers:
                        status_color = "green" if container.get('status') == 'running' else "yellow"
                        click.echo(click.style(f"• {container.get('container_name', 'unknown')}", fg=status_color))
                        click.echo(f"  ID: {container.get('container_id', 'unknown')}")
                        click.echo(f"  Project: {container.get('project_name', 'unknown')}")
                        click.echo(f"  Status: {container.get('status', 'unknown')}")
                        click.echo(f"  Image: {container.get('image', 'unknown')}")
                        click.echo(f"  Started: {container.get('started_at', 'unknown')}")
...
def container_status(ctx, container_id: str):
    ...
    try:
        ...
        if verbose:
            ...
            if response.get('success', False):
                status_color = "green" if response.get('status') == 'running' else "yellow"
...
def container_logs(ctx, container_id: str, tail: int):
    ...
    try:
        client = RacerAPIClient(base_url=api_url, timeout=timeout)
        response = client._make_request('GET', f'/containers/{container_id}/logs?tail={tail}')
