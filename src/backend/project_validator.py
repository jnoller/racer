...
def validate_conda_project(project_path: str) -> Dict[str, Any]:
    ...
    if not project_config.get("name"):
        validation_result["warnings"].append("Project name not specified in conda-project.yml")
    
    if not project_config.get("environments"):
        validation_result["warnings"].append("No environments defined in conda-project.yml")
    
    # Check for common project files
    common_files = ["README.md", "requirements.txt", "setup.py", "pyproject.toml"]
    for file_name in common_files:
        if (project_path / file_name).exists():
            validation_result["warnings"].append(f"Found {file_name} - consider using conda-project instead")
...
def validate_git_repository(git_url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ProjectValidationError(f"Failed to validate git repository {git_url}: {e}")
