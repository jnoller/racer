...
async def generate_dockerfile_endpoint(request: DockerfileGenerationRequest):
    ...
    try:
        ...
        return DockerfileGenerationResponse(
            success=False,
            message=f"Dockerfile generation error: {str(e)}"
        )
...
async def list_projects():
    ...
    try:
        ...
        swarm_services = swarm_response.get("services", []) if isinstance(swarm_response, dict) else []
        ...
        for project in projects:
            project_info = {
                ...
                "started_at": project.created_at.isoformat() if project.created_at else "unknown"
            }
    ...
    try:
        ...
        if not project:
            return ProjectStatusResponse(
                success=False,
                message=f"Project '{request.project_name}' not found"
            )
        ...
        if project["container_id"]:
            container_info = container_manager.get_container_status(project["container_id"])
            if container_info:
                project_info["status"] = container_info["status"]
                project_info["host_ports"] = container_info.get("ports", {})
...
async def list_containers():
    ...
    try:
        ...
        if isinstance(containers_response, dict) and "containers" in containers_response:
            ...
            containers = containers_response if isinstance(containers_response, list) else []
...
async def cleanup_containers():
    ...
    try:
        ...
        return {
            "success": False,
            "message": f"Failed to cleanup containers: {str(e)}"
        }
